...
function generateAnnualComposite(year) {
  ...
  function getMonthlyMedianNDVI(month) {
    var filtered = modisNDVI.filter(ee.Filter.calendarRange(year, year, 'year'))
  ...
  var filteredNDVI = modisNDVI.filter(ee.Filter.date(year + '-01-01', year + '-12-31'))
...
var annualComposite = generateAnnualComposite(2000);

// Export the generated composite to an asset
Export.image.toAsset({
  image: annualComposite,
  description: 'Annual_Composite_2000',
  assetId: 'Annual_Composite_2000',
  scale: 500,
  region: india,
  maxPixels: 1e13
});
...
print('Annual Composite for 2000:', annualComposite);
